...
The app stores task data locally through SwiftData and stores a small set of UI preferences in UserDefaults. Release builds are prepared to use the macOS App Sandbox and a clean zip asset that contains only `MiniDone.app`.
...
- Release app packaging rejects `.xctest`, `.xcresult`, `.dSYM`, `DerivedData`, test-target names, `__MACOSX`, and `._` metadata files inside the final zip.
...
- Release packaging creates a zip containing only `MiniDone.app` bundle contents.
...
- Public GitHub tags always expose GitHub-generated source archives. Users should download the attached `MiniDone-macOS-vX.Y.Z.zip` release asset, not the automatic source-code zip/tar archives.
...
- Confirm the produced zip contains only `MiniDone.app`.
